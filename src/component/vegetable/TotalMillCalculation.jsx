...
import TotalMillCalculationTable from './TotalMillCalculationTable';
...
const TotalMillCalculation = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoader(true);
      await borderMillDataApi(payload);
      setShow(true);
    } catch (error) {
  ...
  return (
    <div>
      <Helmet>
        <title>Milhisab | Daily Mill Calculation </title>
      </Helmet>
      <div className="mx-auto p-6 bg-white rounded-lg shadow-md">
        <h2 className="text-2xl text-center font-semibold text-gray-800 mb-4">
          {new Date().toLocaleDateString()}
        </h2>
        <h2 className="text-2xl font-bold text-center mb-6">Mill Calculation</h2>
...
export default TotalMillCalculation;
